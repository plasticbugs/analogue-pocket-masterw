#!/bin/sh
# Compile the Master of Weapon Pocket core with Quartus 18.1 in Docker (x86 emulation on ARM Macs).
#
...
FLOW=${1:-compile}
case "$FLOW" in
    map) CMD="quartus_map --read_settings_files=on projects/masterw_pocket.qpf -c masterw_pocket" ;;
    compile) CMD="quartus_sh --flow compile projects/masterw_pocket.qpf" ;;
