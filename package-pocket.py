...
RBF = os.path.join(ROOT, "projects", "output_files", "masterw_pocket.rbf")
PKG = os.path.join(ROOT, "pkg", "pocket")
OUT = os.path.join(ROOT, "release", "pocket")
CORE_ID = "plasticbugs.masterw"
...
# release contains everything needed to produce masterw.rom.
for extra in ("masterw.mra", "README.md", os.path.join("tools", "mra_build.py")):
...
print("build the ROM with:  python3 mra_build.py masterw.mra masterw.zip")
